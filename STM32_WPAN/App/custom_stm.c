          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomTx_CharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_TX_CHAR_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_TX_CHAR_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  COPY_DT_SERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomDt_ServHdle));
   ...
   *  DT_TX_CHAR
   */
  COPY_DT_TX_CHAR_UUID(uuid.Char_UUID_128);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : TX_CHAR, error code: 0x%x \n\r", ret);
  }
  else
  {
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TX_CHAR:
      ret = aci_gatt_update_char_value(CustomContext.CustomDt_ServHdle,
                                       CustomContext.CustomTx_CharHdle,
                                       0, /* charValOffset */
                                       SizeTx_Char, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TX_CHAR command, result : 0x%x \n\r", ret);
      }
      else
      {
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TX_CHAR:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomDt_ServHdle, CustomContext.CustomTx_CharHdle, SizeTx_Char, pPayload);
